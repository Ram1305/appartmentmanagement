...
class OverviewTab extends StatelessWidget {
  ...
  const OverviewTab({required this.state, super.key});

  @override
  Widget build(BuildContext context) {
    final totalUsers = state.regularUsers.length;
    final approvedUsers = state.regularUsers.where((u) => u.status == AccountStatus.approved).length;
    final pendingUsers = state.regularUsers.where((u) => u.status == AccountStatus.pending).length;
    final totalBlocks = state.blocks.length;
    // final totalManagers = state.managers.length;
    final totalSecurity = state.securityStaff.length;
      ...
      child: Column(
        ...
        children: [
          _buildStatCard(
            ...
            'Approved: $approvedUsers | Pending: $pendingUsers',
          ),
          ...
          _buildStatCard(
            ...
            '${state.blocks.length} Blocks Created',
          ),
          const SizedBox(height: 16),
          // _buildStatCard(
          //   'Managers',
  ...
  // Manager dialog commented out - manager functionality disabled in admin.
